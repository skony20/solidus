...
| `users` | UNIQUE `(tenant_id, email)`, INDEX `(tenant_id, id)`, FK → `tenants` | E-mail unikalny **w obrębie biura** — ta sama osoba może pracować w dwóch. Kolumny `email_verified_at` + `verification_code_*` obsługują potwierdzanie adresu przy rejestracji (dług techniczny 3) |
...
| `EmailVerificationServiceTest` | PHPUnit + prawdziwy MySQL + `FakeMailer` | Rejestracja wysyła kod i nie aktywuje konta; poprawny kod aktywuje; zły kod zlicza próbę; blokada po 5 próbach; wygaśnięcie kodu; odstęp między wysyłkami; nowy kod unieważnia poprzedni |
...
3. ~~**Brak ekranu rejestracji.**~~ Zrobione. Ekran pod `/rejestracja` (trasa publiczna SPA), rejestracja dwuetapowa: `POST /api/auth/register` zakłada biuro z kontem **niezweryfikowanym**, `POST /api/auth/verify-email` potwierdza adres 6-cyfrowym kodem z maila i od razu loguje (zwraca parę tokenów jak `login`). `POST /api/auth/resend-code` wysyła kod ponownie (odstęp min. 60 s). Kod: 6 cyfr, ważność 15 min, hash w `users.verification_code_hash`, blokada po 5 błędnych próbach. `AuthController::login` odrzuca niezweryfikowane konto z `403` i `reason: email_unverified` — dopiero **po** sprawdzeniu hasła, żeby nie zdradzać istnienia konta. Poczta idzie przez `Shared\Mail\Mailer` (Symfony Mailer, DSN z `MAILER_DSN`; w testach `FakeMailer`) — synchronicznie w cyklu żądania, co dla jednej wiadomości jest do przyjęcia (masowe wysyłki dalej czekają na decyzję o kolejce, sekcja 2.9).
